...
function avoidDrugs() {
  const resultText = document.getElementById('resultText');
  if (energy > 0 && stamina > 0) {
    resultText.textContent = `Avoid ${drugs[level - 1]} it might boost your energy momentarily, but reduces stamina!`;
    resultText.style.color = "#FF0000";
    stamina -= 2;
    energy += 2;
    choseDrug = true;
...
function congratulate() {
  ...
  options.style.display = 'none';
  setTimeout(function() {
    window.location.href = 'http://127.0.0.1:5500/Meme/index.html';
  }, 5000);
